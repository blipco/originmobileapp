...
export function loginEntry(loginObj, navigate) {
    ...
    return {
      type: 'LOGIN_ENTRY',
      payload: axios.post('https://1ed7d901.ngrok.io/api/users/login', { email, password })
              .then ( response => {
              const accessToken = response.data.id;
              const studentId = response.data.userId;
                
            return axios.patch(`https://1ed7d901.ngrok.io/api/users/${studentId}?access_token=${accessToken}`, { deviceId })
              .then(response => {
